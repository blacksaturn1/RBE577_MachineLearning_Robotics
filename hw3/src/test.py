...
class DubinsDataset(Dataset):
    ...
    def _generate_samples(self):
            ...
            for y2 in range(-self.MAX_GRID, self.MAX_GRID + 1, self.X_Y_SPACE):
                for yaw_deg in yaw_values:
                    ...
                    for gamma_deg in gamma_values:
                        ...
                        fname = f"{idx:08d}.npz"
                        fpath = os.path.join("samples", fname)  # relative inside data/
                        disk_path = os.path.join(self.SAMPLES_DIR, fname)
                        # Save compressed
                        np.savez_compressed(disk_path,
                                            traj=traj,
                                            cond=cond,
                                            yaw=np.float32(yaw_rad),
                                            gamma=np.float32(gamma_rad),
                                            quadrant=np.int32(quadrant))
                        index_list.append({
                            "filename": fpath,
        ...
        print(f"Data generation complete. {len(index_list)} samples saved to '{self.SAMPLES_DIR}'")
